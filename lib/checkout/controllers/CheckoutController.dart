...
class CheckoutContoller extends GetxController {
  ...
  addPayment() {
    paymentCheckoutsItems.add(Payment(paymentItems[0].id, paymentItems[0].name,
        0, paymentItems[0].type, 0, 0));
    recalculatePayments();
  }
  ...
  recalculatePayments() {
    ...
    var midValue =
        (double.parse(valueCheckout.value).toInt() - tempBalance) / noBalance;
  ...
  setPayments() {
    ...
    paymentCheckoutsItems.add(Payment(
        paymentItems[0].id,
        paymentItems[0].name,
        double.parse(valueCheckout.value),
        paymentItems[0].type,
        paymentItems[0].balance,
        paymentItems[0].totalPaid));
  }
      ...
      if (data["success"]) {
        ...
        for (var i = 0; i < dataJsonGeneral.length; i++) {
          paymentItems.add(Payment(
              dataJsonGeneral[i]["id"],
              dataJsonGeneral[i]["name"],
              0,
              dataJsonGeneral[i]["method"],
              0,
              0));
