...
class Checkout extends StatelessWidget {
  ...
  canPressPayment() {
    if (controllerCheckout.valueCheckout.value != "") {
      return controllerCheckout.totalCheckout.value >=
          double.parse(controllerCheckout.valueCheckout.value).toInt();
    }
    return false;
  }
  ...
  List<Widget> getPayments(_panelController) {
    ...
    for (var i = 0; i < controllerCheckout.paymentItems.length; i++) {
      print("afasdfasdfasd fasdf asdf ${controllerCheckout.paymentItems[i]}");
              ...
              ? () {
                  if (textContoller.text.length > 0) {
                    ...
                    for (var i = 0; i < controllerCategory.taxes.length; i++) {
                      for (var s = 0;
                          s <
                              controllerCart.controllerCheckboxes.value
                                  .selectedItem.length;
                          s++) {
                        if (controllerCategory.taxes[i].id ==
                            controllerCart
                                .controllerCheckboxes.value.selectedItem[s]) {
                          var taxValue = (double.parse(
                                      "${controllerCategory.taxes[i].rate}") *
                                  tempValue) /
                              100;
                          ...
                          taxLocal.add(TaxCart(
                              controllerCategory.taxes[i].id,
                              "${double.parse("${controllerCategory.taxes[i].rate}")}",
                              controllerCategory.taxes[i].name,
                              "${taxValue}",
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < controllerCategory.taxes.length; i++) {
      taxesText.add(
          "${controllerCategory.taxes[i].name} ${controllerCategory.taxes[i].rate}%");
        ...
        body: Container(
          ...
          child: Column(
            children: [
              Obx(() => GestureDetector(
                    onTap: () {
                      textContoller.text = formatedNumber(
                          "${double.parse(controllerCheckout.valueCheckout.value).toInt()}");
                      controllerCheckout.totalCheckout.value =
                          double.parse(controllerCheckout.valueCheckout.value);
                    },
              ...
              Obx(() => controllerClient.selectedClient.value > 0
                  ? Column(
                      ...
                      children: [
                        ...
                        Container(
                            child: controllerClient.selectedClient.value > 0
                                ? Row(
                                    children: [
                                      InkWell(
                                        onTap: () {
              ...
              Container(
                ...
                child: Obx(() => controllerCheckout.paymentItems.length > 0
                    ? Wrap(
